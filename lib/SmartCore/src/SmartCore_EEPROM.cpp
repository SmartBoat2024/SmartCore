...
namespace SmartCore_EEPROM
{
    ...
    void resetParameters()
    {
        ...
        // ---------------------------------------------------------
        // 3. FAILOVER RESET — INTERNAL RAM STATE
        // ---------------------------------------------------------
        SmartCore_MQTT::pendingBrokerIP = "";
        SmartCore_MQTT::pendingBrokerPort = 0;

        SmartCore_MQTT::currentBrokerIP = "";
        SmartCore_MQTT::currentBrokerPort = 1883;

        SmartCore_MQTT::mqttFailoverAckReceived = false;
        SmartCore_MQTT::failoverInProgress = false;

        SmartCore_MQTT::currentPriorityIndex = 0;

        // ---------------------------------------------------------
        // 4. FAILOVER RESET — EEPROM PRIORITY LIST
        // ---------------------------------------------------------
        mqttPriorityCount = 0;
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            // Also clear RAM
            memset(mqttPriorityList[i], 0, 17);
        }

        // This ensures SmartBox doesn't reuse old “last priority”
        writeByteToEEPROM(MQTT_LAST_PRIORITY_ADDR, 0);

        // ---------------------------------------------------------
        // 5. OPTIONAL: CLEAR BACKUP SMARTBOX LIST (if applicable)
        // ---------------------------------------------------------
        writeIntToEEPROM(0, MQTT_BACKUP_COUNT_ADDR);
        for (int i = 0; i < 8; i++)
        {
            writeStringToEEPROM(
                MQTT_BACKUP_LIST_ADDR + (i * 17),
                "",
                17);
        }
