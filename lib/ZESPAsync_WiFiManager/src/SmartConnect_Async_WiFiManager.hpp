...
#ifndef SmartConnect_Async_WiFiManager_hpp
#define SmartConnect_Async_WiFiManager_hpp
...
#include "SmartConnect_Async_WiFiManager_Debug.h"
...
"h2{font-size: 2em; color: white; text-align: center; margin-top: 10px;} "
"h1{font-size: 3em} "
".btn{background: #009473;border-radius: 8px;border: 0;color: #fff;cursor: pointer; font-size: 1.2em; display: inline-block;margin: 2px 0;padding: 10px 14px 11px;width: 100%} "
".btn:hover{background: #013220} "
".btn:active,.btn:focus{background: #013220} "
"label>*{display: inline} "
"form>*{display: block;margin-bottom: 10px;background-color: #333;padding: 10px;border-radius: 6px;} "
"input, textarea, select {background-color: #fff !important; color: #000 !important;} "
...
"small { color: white; font-size: 0.9em; display: block; margin: 10px 0; } "
".network-entry{display:flex;justify-content:space-between;align-items:center;background-color:#333;color:#0074cc;padding:6px;margin-bottom:6px;border-radius:6px;} "
".network-entry a.ssid{background-color:white;color:#0074cc;padding:3px 6px;border-radius:4px;text-decoration:none;flex-grow:1;font-weight:bold;overflow-wrap:anywhere;} "
".lock-icon{color:white;margin-left:6px;min-width:60px;text-align:right;} "
".lock-icon.l::before{content:'\\1F512';font-family:\"Segoe UI Emoji\",\"Noto Color Emoji\",sans-serif;margin-right:4px;} "
".network-label{font-size: 1.0em; margin-bottom:10px; color:rgb(255, 255, 255);} "
"</style>";


////////////////////////////////////////////////////

const char WM_HTTP_SCRIPT[] PROGMEM = R"rawliteral(
  <script>
  function c(l) {
    const ssid = l.innerText || l.textContent;
  
    // Set SSID input
    const ssidInput = document.getElementById('s');
    if (ssidInput) ssidInput.value = ssid;
  
    // Focus password input (with cursor)
    const passwordInput = document.getElementById('p');
    if (passwordInput) {
      setTimeout(() => {
        passwordInput.focus();
        passwordInput.select();  // 🔥 This ensures cursor is visible
      }, 100);
    }
    ...
    if (typeof timezone !== 'undefined') {
      ...
      if (tzField) {
        tzField.value = timezone.name();
        if (tzField.innerHTML !== undefined) {
          tzField.innerHTML = timezone.name();
        }
      }
    }
  }
...
const char WM_HTTP_HEAD_END[] PROGMEM = "</head><body class='carbon-fiber'><div class='container'><div style='text-align:left;display:inline-block;min-width:260px;'>";
...
const char WM_HTTP_PORTAL_OPTIONS[] PROGMEM = 
  "<form action='/wifi' method='get'><button class='btn'>Configuration</button></form><br/>"
  "<form action='/i' method='get'><button class='btn'>Information</button></form><br/>";

const char CUSTOM_PORTAL_BUTTONS[] PROGMEM = 
  "<form action='/smartconnect' method='get'><button class='btn'>SmartConnect</button></form><br/>"
  "<form action='/standalone' method='get'><button class='btn'>Standalone</button></form><br/>";

const char WM_HTTP_PORTAL_EXIT[] PROGMEM = 
  "<form action='/close' method='get'><button class='btn'>Exit Portal</button></form><br/>";

const char WM_HTTP_LIST_START[] PROGMEM = 
  "<div class='entry-container'>"
    "<p class='network-label'>Available Networks</p>";

    
const char WM_HTTP_PARAM_START[] PROGMEM = 
  "<div class='entry-container'>"
    "<p class='network-label'>Please enter your Details</p>";

const char WM_HTTP_STATIC_START[] PROGMEM = 
    "<div class='entry-container'>"
      "<p class='network-label'>For custom setup only - SET STATIC IP </p>";

const char WM_HTTP_ITEM[] PROGMEM = 
  "<div class='network-entry'>"
    "<a class='ssid' href='#p' onclick='c(this)'>{v}</a>"
    "<span class='lock-icon {i}'>{r}%</span>"
  "</div>";
...
const char WM_HTTP_FORM_START[] PROGMEM = "<form method='get' action='wifisave'><fieldset><div><label>SSID</label><input value='[[ssid]]' id='s' name='s' length=32 placeholder='SSID'><div></div></div><div><label>Password</label><input value='[[pwd]]' id='p' name='p' length=64 placeholder='password'><div></div></div><div><label>Backup SSID</label><input value='[[ssid1]]' id='s1' name='s1' length=32 placeholder='Backup SSID'><div></div></div><div><label>Backup SSID Password</label><input value='[[pwd1]]' id='p1' name='p1' length=64 placeholder='Password'><div></div></div></fieldset>";
#else
...
#define RFC952_HOSTNAME_MAXLEN      24
    ...
    void          handleSmartConnect(AsyncWebServerRequest *request);
    void          handleStandalone(AsyncWebServerRequest *request);
    void          handleSetSerialNumber(AsyncWebServerRequest *request);
    void          handleSetStandaloneAP(AsyncWebServerRequest *request);
    void          handleAutoSmartConnect(AsyncWebServerRequest *request);
